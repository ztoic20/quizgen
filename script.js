...
(() => {
  const fileEl = document.getElementById("xlsxFile");
  const loadBtn = document.getElementById("btnLoadXLSX");
  const csvEl = document.getElementById("csvFile");
  const loadCSVBtn = document.getElementById("btnLoadCSV");
  const useDefBtn = document.getElementById("btnUseDefault");
  const msg = document.getElementById("importMsg");
  if (!fileEl || !loadBtn || !useDefBtn || !loadCSVBtn || !csvEl) return;
  ...
  loadBtn.addEventListener("click", async () => {
    try {
      ...
      if (!f) { setMsg("Please choose an .xlsx file first."); return; }
      setMsg("Loading XLSX…");
      if (!window.parseQuestionsFromXLSX) throw new Error("XLSX importer not loaded.");
      const qs = await window.parseQuestionsFromXLSX(f);
      setMsg(`Loaded ${qs.length} questions from XLSX.`);
      setQuestionSet(qs, "Custom Quiz (XLSX)");
    } catch (err) {
  ...
  loadCSVBtn.addEventListener("click", async () => {
    try {
      ...
      setQuestionSet(qs, "Custom Quiz (CSV)");
    } catch (err) {
      setMsg(String(err && err.message ? err.message : err));
      console.error(err);
    }
  ...
  useDefBtn.addEventListener("click", () => {
    setMsg("Using default SQLite questions.");
    setQuestionSet(RAW, "SQLite Quiz – Week 4 (50 Items)");
  });
})();
